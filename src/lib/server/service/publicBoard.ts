...
export const publicBoardService = {
	...
	update: async (id: string, item: Partial<Omit<Project, 'id' | 'organizationId' | 'isPublic'>>) => {
	...
	addTask: async (projectId: string, taskData: { name: string; description?: string; statusId?: string }) => {
		const id = crypto.randomUUID();
