...
export const paymentRepository = {
	...
	update: async (id: string, input: Partial<Omit<PaymentInsert, 'id'>>) => {
		const payload: Partial<Omit<PaymentInsert, 'id'>> = {
			...input,
			updatedAt: now()
		};
	...
	updateStatus: async (
		...
		const updates: Partial<Omit<PaymentInsert, 'id'>> = {
			status,
...
`;
		...
		return {
			...
			revenueSummary: analytics?.revenue_summary ?? {
				total_revenue: '0',
				pending_value: '0',
