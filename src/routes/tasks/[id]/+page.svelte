...
<div class="p-4">
	...
	<form method="POST" action="?/updateTask" class="space-y-4">
		<div>
			...
			<input type="text" name="name" id="taskName" value={data.task.name} class="input input-bordered w-full" />
		</div>
		<div>
			...
			<select name="projectId" id="projectId" class="select select-bordered w-full">
				{#each data.projects as project}
					<option value={project.id} selected={project.id === data.task.projectId}>{project.name}</option>
				{/each}
			</select>
		</div>
		<div>
			...
			<select name="assigneeId" id="assigneeId" class="select select-bordered w-full">
				{#each data.users as user}
					<option value={user.id} selected={user.id === data.task.assigneeId}>{user.username}</option>
				{/each}
			</select>
		</div>
		<div>
			...
			<select name="statusId" id="statusId" class="select select-bordered w-full">
				{#each data.taskStatuses as status}
					<option value={status.id} selected={status.id === data.task.statusId}>{status.name}</option>
				{/each}
			</select>
		</div>
