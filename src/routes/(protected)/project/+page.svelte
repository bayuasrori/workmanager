<script lang="ts">
	import { deleteProject, getProjects } from './data.remote';
	...
	const handleDeleteProject = async (projectId: string) => {
		...
		try {
			await deleteProject({ projectId }).updates(getProjects());
		} catch (error) {
...
<div class="min-h-[70vh] bg-gradient-to-br from-emerald-50 via-emerald-100 to-amber-50 py-10">
	<div class="mx-auto w-full max-w-6xl px-4">
			...
			<section class="mt-8 grid gap-6 lg:grid-cols-[minmax(0,_1fr)_minmax(0,_2fr)]">
				<div class="card border border-emerald-200/70 bg-white/90 shadow-2xl backdrop-blur">
					<div class="card-body gap-3 p-6">
						<h2 class="text-sm font-semibold uppercase tracking-wide text-emerald-900">
							Ringkasan
						</h2>
				...
				<div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
					{#each projects as project (project.id)}
						<article
							class="group card border border-emerald-200/60 bg-white/90 shadow-xl transition hover:-translate-y-1 hover:shadow-2xl"
						>
							<div class="card-body gap-4 p-6">
								<div class="flex items-start justify-between gap-3">
									<div>
										<h2 class="text-xl font-semibold text-emerald-900">{project.name}</h2>
										<p class="text-xs uppercase tracking-wide text-emerald-700/70">
											ID: {project.slug}
										</p>
									</div>
								</div>
								...
								<div class="mt-auto flex flex-wrap gap-2">
									...
									<a
										...
										class="btn btn-sm btn-ghost text-emerald-700 hover:bg-emerald-100"
									>
										Atur Proyek
									</a>
									<button
										type="button"
										class="btn btn-sm btn-outline border-rose-200 text-rose-600 hover:bg-rose-50"
		...
		{:else}
			<section class="mt-12">
				<div
					class="card border border-dashed border-emerald-200 bg-white/90 py-12 text-center shadow-xl"
				>
