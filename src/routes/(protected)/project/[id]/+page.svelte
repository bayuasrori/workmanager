<script lang="ts">
	...
	let isMakingPublic = $state(false);
	...
	async function handleUpdateProject(event: Event) {
		...
		try {
			...
			}).updates(getProjectData({ projectId }));
	...
	async function handleRemoveMember(userId: string, username?: string) {
		if (!confirm(`Hapus ${username || 'anggota'} dari proyek ini?`) || !projectId) return;
	...
	async function handleMakePublic() {
		...
		if (
			!confirm(
				'Buat papan publik untuk proyek ini? Papan akan dapat diakses melalui link dan siapa saja bisa melihat dan mengedit tugas.'
			)
		) {
			return;
		}
		...
		isMakingPublic = true;
		try {
			const result = await makePublicForUser({
				projectId
			}).updates(getProjectData({ projectId }));
			if (result?.slug) {
				alert(`Papan publik berhasil dibuat! Link: /public-board/${result.slug}`);
				// Optional: navigate to public board
				// window.location.href = `/public-board/${result.slug}`;
			}
		} catch (error) {
			console.error('Failed to make project public', error);
			alert('Gagal membuat papan publik. Silakan coba lagi.');
		} finally {
	...
	async function handleDeleteTaskStatus(statusId: string, statusName: string) {
		if (
			!confirm(`Hapus status "${statusName}"? Tugas dengan status ini tidak akan dihapus.`) ||
			!projectId
		)
			return;
		...
		try {
			await deleteTaskStatus({ statusId }).updates(getProjectData({ projectId }));
		} catch (error) {
				...
				<div class="card border border-emerald-200/70 bg-white/90 shadow-2xl backdrop-blur">
					<div class="card-body p-8">
						...
						<p class="text-sm text-emerald-700/70">
							Informasi ini membantu anggota tim memahami ruang kerja.
						</p>
						<form onsubmit={handleUpdateProject} class="mt-6 grid gap-6">
							...
							>
								<a href="/project" class="btn btn-ghost text-emerald-600 hover:bg-emerald-100"
									>Kembali</a
								>
				...
				<div class="space-y-6">
					<div class="card border border-emerald-200/70 bg-white/90 shadow-xl backdrop-blur">
						<div class="card-body p-6">
							...
							{#if projectMembers.length}
								<ul class="mt-4 space-y-3">
									{#each projectMembers as member (member.userId)}
										<li
											class="flex items-center justify-between gap-3 rounded-lg border border-emerald-100 bg-emerald-50/70 px-4 py-3"
										>
											<div>
											...
											<button
												...
												onclick={() =>
													handleRemoveMember(member.userId, member.username ?? member.email)}
											>
							...
							{:else}
								<div
									class="mt-4 rounded-lg border border-dashed border-emerald-200 bg-emerald-50/40 px-4 py-6 text-center text-sm text-emerald-700/70"
								>
					...
					<div class="card border border-emerald-200/70 bg-white/90 shadow-xl backdrop-blur">
						<div class="card-body p-6">
							...
							<p class="text-sm text-emerald-700/70">
								Pilih pengguna untuk bergabung. Mereka akan langsung memiliki akses ke papan tugas
								proyek ini.
							</p>
					...
					<div class="card border border-emerald-200/70 bg-white/90 shadow-xl backdrop-blur">
						<div class="card-body p-6">
							...
							<p class="text-sm text-emerald-700/70">Kelola status tugas di proyek ini.</p>
